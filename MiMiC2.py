...
parser.add_argument('--models', metavar='{INPUT}', required=True, help='Folder containing metabolic models for each genome. Must be provided as RDS files such as those provided by GapSeq.')
...
parser.add_argument('--exclusion', metavar='{INPUT}', required=False, help='Provide file which includes a csv list of genome names to be excluded during SynCom selection.')
...
print ('Number of pfams studied; ' + str(len(pfams)))
...
## Identify core proteins for each group
...
outputting_all_Scores = open(location_of_call + '/'+ output_prefix + '-Consortia_scores.tab','w')

outputting_all_Scores.write('SynCom members\tGroup score\n')
for k,v in consortia_mean_Scores.items():
	outputting_all_Scores(k + '\t' + str(v) + '\n')
...
while passed_phase_two_stability == False:
    ...
    if consortia_size_wanted == 2:
        ...
        outputting = open(location_of_call + '/'+ output_prefix + '-Community-wide_response.txt','w')

        outputting.write('#Species\tConsortia growth Vs Single growth (%)\n')
    ...
    if passed_phase_one_stability == False or passed_phase_two_stability == False: # The community predicted isnt good, so another round is needed
        ...
        subprocess.run(['mv',location_of_call + '/'+ output_prefix +'-Paired_Interactions.txt',location_of_call + '/'+ output_prefix + 'Modelling-iteration-' + str(modelling_iteration)])
        subprocess.run(['mv',location_of_call + '/'+ output_prefix +'-Community-wide_response.txt',location_of_call + '/'+ output_prefix + 'Modelling-iteration-' + str(modelling_iteration)])
...
outputting_taxonomy = open(location_of_call + '/'+ output_prefix + '-SynCom_Taxonomy.tab','w')
...
for sp in species:
    growth = []
    for time in times:
        growth.append(int(df.loc[df['data.species'] == sp].loc[df['data.time'] == time]['data.value']))
    raw_data[sp] = growth
